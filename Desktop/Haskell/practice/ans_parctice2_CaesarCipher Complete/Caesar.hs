...
histogram xs =
   map (\c -> 100 *.
            fromIntegral (count c xs) /
               fromIntegral n) alpha
  where alpha = ['a'..'z']
        n = lowers xs
...
crack xs = pos (minimum similarities) similarities
   where histo = histogram xs
         rotates = map (\i -> rotate i histo) [0..25]
         similarities = map (chisqr table) rotates
